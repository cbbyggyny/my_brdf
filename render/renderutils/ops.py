...
#----------------------------------------------------------------------------
# C++/Cuda plugin compiler/loader.  编译器/加载器
...
def _get_plugin():
    ...
    if os.name == 'nt':     # nt代表win系统
        def find_cl_path():
            import glob
            for edition in ['Enterprise', 'Professional', 'BuildTools', 'Community']:
                paths = sorted(glob.glob(r"C:\Program Files (x86)\Microsoft Visual Studio\*\%s\VC\Tools\MSVC\*\bin\Hostx64\x64" % edition), reverse=True)   # 我的2022没有（X86）
                if paths:
                    return paths[0]
                
        # If cl.exe is not on path, try to find it.
        if os.system("where cl.exe >nul 2>nul") != 0:   # >nul 用于将标准输出重定向到空，而 2>nul 用于将标准错误输出也重定向到空
...
class _fresnel_shlick_func(torch.autograd.Function):
    ...
    def backward(ctx, dout):
        ...
        return _get_plugin().fresnel_shlick_fwd(f0, f90, cosTheta, dout) + (None,)
...
def _fresnel_shlick(f0, f90, cosTheta, use_python=False):
    ...
    if torch.is_anomaly_enabled():  # 检查是否启用了异常检测
        assert torch.all(torch.isfinite(out)), "Output of _fresnel_shlick contains inf or NaN"  # 检查张量 out 中的所有元素是否为有限值（finite）
    return out
...
def _ndf_ggx(alphaSqr, cosTheta, use_python=False):
    ...
    if torch.is_anomaly_enabled():
        assert torch.all(torch.isfinite(out)), "Output of _ndf_ggx contains inf or NaN"  # 检查张量 out 中的所有元素是否为有限值（finite）
    return out
...
def __ndfBounds(res, roughness, cutoff):
    def ndfGGX(alphaSqr, costheta):
        ...
        return alphaSqr / (d * d * np.pi)   # 数学原理？
    
    # Sample out cutoff angle
    nSamples = 1000000 # 一百万
    costheta = np.cos(np.linspace(0, np.pi/2.0, nSamples))
    D = np.cumsum(ndfGGX(roughness**4, costheta))   # np.cumsum将数组中的每个元素与前面的元素相加，形成一个新的数组
    idx = np.argmax(D >= D[..., -1] * cutoff)   # 查找数组 D 中第一个>= D 的最后一个元素* cutoff 的值的元素，并返回该元素的索引。
...
def specular_cubemap(cubemap, roughness, cutoff=0.99, use_python=False):
    ...
    if use_python:
        ...
        if key not in __ndfBoundsDict:
            __ndfBoundsDict[key] = __ndfBounds(*key)    # *key 表示将 key 拆解为多个参数传递给函数
